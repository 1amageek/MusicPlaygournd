...
struct LiveReplacementPlaybackTests {
    ...
    func scratchRendersReversePCMWhilePausedAndRestoresTransportIntent() throws {
        ...
        try transport.scratch(bySeconds: -0.02, over: 0.01)
        #expect(!transport.snapshot().isPlaying && transport.isScratching)
        let reverse = try render(transport, frames: 1024)
        #expect(reverse.allSatisfy { $0.isFinite })
        #expect(reverse.contains { abs($0) > 0.01 })
        #expect(transport.snapshot().beatPosition < start)
        ...
        #expect(try render(transport, frames: 8).allSatisfy { $0 == 0 })
        try transport.startPlayback()
        let forwardStart = transport.snapshot().beatPosition
        try transport.scratch(bySeconds: 0.01, over: 0.02)
        #expect(try render(transport, frames: 1024).allSatisfy { $0.isFinite })
        #expect(transport.snapshot().beatPosition > forwardStart)
        #expect(transport.snapshot().isPlaying)
        transport.endScratch()
        _ = try render(transport, frames: 256)
        ...
        #expect(try render(transport, frames: 1024).contains { abs($0) > 0.01 })
        transport.endScratch()
        _ = try render(transport, frames: 256)
    }
