...
- [x] SYNTH-1 Borrow active voices for frame advancement; sampled per-frame aggregate copies removed without changing DSP arithmetic. 24 release tests passed including exact 1/8/16-track PCM, voice stealing/choke and seamless boundaries. Clean median 4s render: 8 tracks 247.83 to 69.77 ms; 16 tracks 489.66 to 136.83 ms; 7 runs each. Commit: Avoid per-frame voice copies during synthesis `depends:none` `parallel:none`
- [ ] SYNTH-INT Verify affected rendering/native paths, review scope, commit and push only task changes to configured main upstream `depends:SYNTH-1` `parallel:none`
