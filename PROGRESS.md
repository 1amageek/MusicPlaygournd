...
- [x] ISSUES-1 Snapshot-backed compiler records, gutter/underline, subtle row tint, trailing inline messages and error-first navigable issue list implemented. Four focused tests passed in 3.988s: real compiler failures/cross-file navigation, Unicode/ambiguity/EOF, native annotation lifecycle and retained audio after typed/raw failures. Updated obsolete workflow fixture to open a document and reuse the prepared SDK. MainActor presentation ownership and unchanged Evaluation failure contract reviewed; full build output retained collapsed `depends:none` `parallel:none`
- [ ] ISSUES-INT Verify actual compiler errors, cross-file navigation, Unicode positions, stale edits, native annotations and packaged UI; commit and push task changes `depends:ISSUES-1` `parallel:none`
