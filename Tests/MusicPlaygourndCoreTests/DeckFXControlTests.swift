...
extension NativeHostTests {
    @MainActor struct DeckFXControlTests {
        ...
        func beatSyncFreeRateResetAndPad() async throws {
            ...
            settings.enabled = true; settings.rate = 3; settings.feedback = 0.6
            ...
            #expect(model.fxSettings.enabled && model.fxSettings.rate == 3 && model.fxSettings.feedback == 0.6)
            ...
            #expect(model.fxSettings.mix == 1 && model.fxSettings.depth == 1)
            model.resetFX()
            #expect(!model.fxSettings.enabled && model.fxBeats == 1)
