...
public enum RenderWorker {
    ...
    public static func runPrepared(
        ...
        let switchBank = preparation.switchBank
        if let switchBank {
            guard preparation.switchSessions.count == switchBank.variants.count,
                  preparation.switchSessions.allSatisfy({ $0.revision == revision }),
                  preparation.switchSessions.indices.contains(switchBank.initialIndex),
                  preparation.switchSelection != nil else {
                throw EvaluationError.invalidResult("Compiled switch sessions do not match their bank.")
            }
            guard preparation.switchSessions.enumerated().allSatisfy({ index, session in
                session.baseline == switchBank.variants[index].loop
                    && switchBank.variants[index].metadata.revision == revision
                    && switchBank.variants[index].catalog.descriptors.allSatisfy { $0.address.revision == revision }
            }) else {
                throw EvaluationError.invalidResult("Compiled switch initial loop does not match its bank.")
            }
        } else {
            guard preparation.switchSessions.isEmpty, preparation.switchSelection == nil else {
                throw EvaluationError.invalidResult("Switch sessions require a prepared switch bank.")
            }
        }
        ...
        if let switchBank {
            try publishSwitchBank(switchBank, outputURL: outputURL)
            for (index, variant) in switchBank.variants.enumerated() {
                try publish(variant, index: index, revision: revision, to: outputURL)
            }
        }
        ...
        let state = RenderWorkerState(session: session, revision: revision, outputURL: outputURL,
                                      ...
                                      switchSessions: preparation.switchSessions,
                                      switchSelection: preparation.switchSelection,
                                      writer: writer)
        while true {
            ...
            case .selectSwitchVariant(let commandRevision, let generation, let operationID, let index):
                try await state.selectSwitchVariant(
                    revision: commandRevision,
                    generation: generation,
                    operationID: operationID,
                    index: index
    ...
    private static func publishSwitchBank(_ bank: PreparedSwitchBank, outputURL: URL) throws {
        let encoder = PropertyListEncoder()
        encoder.outputFormat = .binary
        let data = try encoder.encode(RenderWorkerSwitchDescriptor(bank: bank))
        guard data.count <= maximumResultBytes else {
            throw EvaluationError.invalidResult("Worker switch metadata exceeds 16 MiB.")
        }
        let file = outputURL.deletingLastPathComponent().appending(path: "prepared-switch-bank.plist")
        try data.write(to: file, options: .atomic)
    }

    private static func publish(
        _ variant: SwitchVariant,
        index: Int,
        revision: UInt64,
        to outputURL: URL
    ) throws {
...
private actor RenderWorkerState {
    ...
    private let switchBank: PreparedSwitchBank?
    private let switchSessions: [LoopRenderSession]
    private let switchSelection: (@MainActor @Sendable ([Int]) -> Void)?
    private var activeSwitchIndex: Int
    ...
    init(
        ...
        switchBank: PreparedSwitchBank?,
        switchSessions: [LoopRenderSession],
        switchSelection: (@MainActor @Sendable ([Int]) -> Void)?,
        ...
        self.activeSwitchIndex = switchBank?.initialIndex ?? 0
    ...
    func selectSwitchVariant(
        ...
        guard let switchBank,
              switchSessions.indices.contains(index),
              switchBank.variants.indices.contains(index) else {
            try await writer.send(.switchSelected(
                revision: commandRevision, generation: generation, operationID: operationID, index: index, accepted: false
            ))
            return
        }
        guard commandRevision == revision, operationID > latestOperationID else {
            try await writer.send(.switchSelected(
                revision: commandRevision, generation: generation, operationID: operationID, index: index, accepted: false
            ))
            return
        ...
        await cancelActiveWorkForSwitch()
        if let switchSelection {
            await switchSelection(switchBank.variants[index].selection)
        }
        session = switchSessions[index]
        activeSwitchIndex = index
        metadata = switchBank.variants[index].metadata
        performanceControls = []
        acceptedPerformanceValues = [:]
        acceptedPerformanceModelID = nil
        acceptedPerformanceGeneration = 0
        latestPerformanceGeneration = 0
        pendingPerformance = nil
        try await writer.send(.switchSelected(
            revision: commandRevision, generation: generation, operationID: operationID, index: index, accepted: true
        ))
        startPendingExportIfAvailable()
    }

    private func cancelActiveWorkForSwitch() async {
        pendingRender = nil
