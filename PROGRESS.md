...
- [x] TCUE-1 Per-deck MainActor CUE, 30px native circular press/release button, Option sides and Shift return implemented; headphone routing unchanged. Four focused tests passed in 0.707s, including native output PCM, cue return, held preview, cancellation, independent deck playback, keyboard chords and queued transport. Current main behavior documented separately from released 0.5.1 `depends:none` `parallel:none`
- [ ] TCUE-INT Build and inspect integrated app, update shortcut documentation, commit and push verified task changes `depends:TCUE-1` `parallel:none`
