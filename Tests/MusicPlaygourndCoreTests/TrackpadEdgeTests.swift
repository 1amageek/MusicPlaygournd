...
extension NativeHostTests {
    ...
    struct TrackpadEdgeTests {
        ...
        func edgeRoutingLocksContactsAndSeparatesReleaseFromCancellation() {
            ...
            #expect(TrackpadEdgeController.region(at: NSPoint(x: 0.18, y: 0.12)) == .crossfade)
            #expect(TrackpadEdgeController.region(at: NSPoint(x: 0.82, y: 0.12)) == .crossfade)
            #expect(TrackpadEdgeController.region(at: NSPoint(x: 0.5, y: 0.121)) == nil)
