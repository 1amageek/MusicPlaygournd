...
- [x] SCRATCH-1 Continuous velocity/hold/stop and 5ms lifecycle joins with bounded sinc reads; 23 focused checks passed, then three native stopped-output/cancellation checks passed. Alias fixtures 1.5x/4x/-4x/32x rejected 80.6...81.2dB; reversal jump 0.0188 versus prior gap 0.191; two-deck 32x 512-frame median 2.715ms versus 11.61ms source budget on M4 Max. ASan passed 96 wrapped/short-buffer reads. Reviewed immutable PCM/table borrows, Mutex isolation, target bounds, native failure cleanup and parent contract `depends:none` `parallel:none`
- [ ] SCRATCH-INT Run scratch/adoption/clock integration checks, build and launch signed app, verify only task commits and push configured upstream `depends:SCRATCH-1` `parallel:none`
