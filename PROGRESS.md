...
- [x] ISSUES-1 00fc5f0: Snapshot-backed compiler records, gutter/underline, subtle row tint, trailing inline messages and error-first navigable issue list implemented. Four focused tests passed in 3.988s: real compiler failures/cross-file navigation, Unicode/ambiguity/EOF, native annotation lifecycle and retained audio after typed/raw failures. Updated obsolete workflow fixture to open a document and reuse the prepared SDK. MainActor presentation ownership and unchanged Evaluation failure contract reviewed; full build output retained collapsed `depends:none` `parallel:none`
- [x] ISSUES-INT Four focused tests passed in 3.988s; production app built in 75.44s and strict signature verified. Real project compiler error shown at line 7/column 19 with red gutter, row tint and trailing message; native inline button selected the failing character and opened error-first details with collapsed build output. No user song sources changed `depends:ISSUES-1` `parallel:none`
...
- [x] WEB-1 510a470: Implemented approved static landing page with exact logo crop and real running A/B template capture. Desktop 1280px and mobile 390px inspected; no horizontal overflow, all images loaded, source-install anchor and keyboard focus verified. Runnable check passes README command parity, relative assets and clipboard success/denied/unavailable paths; app sources unchanged `depends:none` `parallel:none`
- [x] WEB-INT Committed page verified at /docs/: all actual images loaded at desktop 1280px and mobile 390px with no horizontal overflow. Anchor navigation, keyboard focus and clipboard status verified in browser; source-only install parity and clipboard failure checks passed. GitHub Pages main /docs setup documented; public hosting remains disabled. Only task commits are eligible for normal upstream push `depends:WEB-1` `parallel:none`

- [x] TCUE-1 b45a2bc: Per-deck MainActor CUE, 30px native circular press/release button, Option sides and Shift return implemented; headphone routing unchanged. Four focused tests passed in 0.707s, including native output PCM, cue return, held preview, cancellation, independent deck playback, keyboard chords and queued transport. Current main behavior documented separately from released 0.5.1 `depends:none` `parallel:none`
- [x] TCUE-INT Release preview app built and strict signature verified; native UI inspected with LiveSet051, confirming both 30px circular CUE buttons beside transport and unchanged header height. Four focused tests verify PCM preview, release return and keyboard routing; README distinguishes main from release 0.5.1. Task changes reviewed for normal main upstream push `depends:TCUE-1` `parallel:none`

- [x] FX-1 Independent live Phaser/Chorus/Flanger, validated smoothed settings and deck-colored FX panels implemented. Five focused native tests passed in 0.835s: PCM changes, partition invariance, bypass/reset, parameter changes, invalid retention, A/B isolation and compressor/CUE regression. Native UI verifies selection, sliders, reset and both buttons fitting the 1160px window `depends:none` `parallel:none`
- [ ] FX-INT Build and visually inspect the app, record focused native tests and commit verified changes for upstream push `depends:FX-1` `parallel:none`
