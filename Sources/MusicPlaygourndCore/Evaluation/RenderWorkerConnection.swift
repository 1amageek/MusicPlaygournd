...
internal actor RenderWorkerConnection {
    ...
    private var switchWaiter: CheckedContinuation<Bool, any Error>?
    ...
    private var switchDeadline: ContinuousClock.Instant?
    ...
    private var latestSwitchOperationID: UInt64 = 0
    private var latestSwitchGeneration: UInt64 = 0
    private var latestSwitchIndex: Int = 0
    ...
    func selectSwitchVariant(index: Int, generation: UInt64 = 0) async throws -> Bool {
        try Task.checkCancellation()
        guard let switchBank,
              switchBank.variants.indices.contains(index) else {
            throw EvaluationError.invalidResult("This worker has no selected Swift switch variant.")
        }
        if let failure { throw failure }
        guard readyResult != nil, !stopping, !closing else {
            throw EvaluationError.invalidResult("Worker is unavailable.")
        }
        guard switchWaiter == nil else {
            throw EvaluationError.invalidResult("A Swift switch selection is already pending.")
        }
        let operationID = try allocateOperationID()
        latestSwitchOperationID = operationID
        latestSwitchGeneration = generation
        latestSwitchIndex = index
        let frame = try RenderWorkerFraming.encode(RenderWorkerCommand.selectSwitchVariant(
            revision: revision, generation: generation, operationID: operationID, index: index
        ))
        try Task.checkCancellation()
        switchDeadline = .now.advanced(by: .seconds(10))
        return try await withCheckedThrowingContinuation { continuation in
            switchWaiter = continuation
            enqueue(.selectSwitch, frame)
        }
    }
    ...
    private func closeWorker() async {
        ...
        switchWaiter?.resume(throwing: CancellationError()); switchWaiter = nil
        pendingPerformance = nil
    ...
    private func abort(_ error: EvaluationError) async {
        ...
        switchWaiter?.resume(throwing: error); switchWaiter = nil
        pendingPerformance = nil
    ...
    private func pump() async {
        while !stopping {
            do {
                ...
                let deadlines = [readyDeadline, renderDeadline, exportDeadline, visualizationDeadline,
                                 performanceDeadline, performanceCommandDeadline, switchDeadline,
                                 protocolCloseDeadline].compactMap { $0 }
    ...
    private func receive(_ response: RenderWorkerResponse) throws {
        ...
        case .ready(let responseRevision, let catalog, let performanceControls):
            ...
            let bank = try readSwitchBank()
            if let bank {
                guard bank.variants.allSatisfy({ variant in
                    variant.metadata.revision == revision
                        && variant.catalog.descriptors.allSatisfy { $0.address.revision == revision }
                }),
                      bank.variants[bank.initialIndex].loop == result.loop,
                      bank.variants[bank.initialIndex].metadata == metadata,
                      bank.variants[bank.initialIndex].catalog == catalog else {
                    throw EvaluationError.invalidResult("Worker initial switch variant does not match the ready result.")
                }
            }
        ...
        case .switchSelected(let responseRevision, let generation, let operationID, let index, let accepted):
            guard responseRevision == revision, operationID <= latestOperationID else {
                throw EvaluationError.invalidResult("Invalid worker switch selection acknowledgement.")
            }
            guard operationID == latestSwitchOperationID else { return }
            guard generation == latestSwitchGeneration, index == latestSwitchIndex else {
                throw EvaluationError.invalidResult("Worker switch selection acknowledgement does not match its request.")
            }
            guard let switchBank, switchBank.variants.indices.contains(index) else {
                throw EvaluationError.invalidResult("Invalid worker switch selection index.")
            }
            switchDeadline = nil
            if accepted {
                let variant = switchBank.variants[index]
                readyResult = RetainedEvaluation(
                    loop: variant.loop,
                ...
                lastRenderedGeneration = max(lastRenderedGeneration, generation)
            }
            switchWaiter?.resume(returning: accepted)
            switchWaiter = nil
        ...
        case .failed(let responseRevision, _, let operationID, let message):
            ...
            } else if operationID == latestSwitchOperationID {
                switchDeadline = nil
                switchWaiter?.resume(throwing: EvaluationError.processFailed(message)); switchWaiter = nil
            } else {
                return
            }
    ...
    private func readSwitchBank() throws -> PreparedSwitchBank? {
        let directory = outputURL.deletingLastPathComponent()
        let descriptorURL = directory.appending(path: "prepared-switch-bank.plist")
        guard FileManager.default.fileExists(atPath: descriptorURL.path) else { return nil }
        let descriptorData = try boundedData(at: descriptorURL)
        let descriptor = try PropertyListDecoder().decode(RenderWorkerSwitchDescriptor.self, from: descriptorData)
        guard descriptor.selections.count <= PreparedSwitchBank.maximumVariants else {
            throw EvaluationError.invalidResult("Worker switch descriptor exceeds the variant bound.")
        }
