...
## Related Designs
The independently served [landing page](docs/DESIGN.md) owns product presentation and installation navigation.
...
Run core behavioral tests, real compiler good/bad/recovery checks, AVAudioEngine output checks and live UI. Scripts/build-app.sh bundles source for evaluation and records the installed Swift executable. App runtime needs Swift 6.4 and Xcode command-line tools including Python3. Version 0.3.0 is a source preview release; no prebuilt notarized binary or App Store distribution is provided.
...
One native audio graph owns main output, the compressor, recording and master analyzers. Optional headphone routing is owned by [Playback](Sources/MusicPlaygourndCore/Playback/DESIGN.md#headphone-cue). A deck pause stops only its source transport; the other deck and output graph continue. The mixer uses equal-power crossfade endpoints with explicit exact zero at the opposite endpoint; deck gain is independent. Gain transitions reuse bounded smoothing. Visualization does not modify audio width; mono has zero Side, while the DJ view also projects time as depth. Standalone master monitoring consumes post-mix stereo output; the central DJ scope presents the two pre-mix deck captures independently, as specified by the Editor design. There is no extra long PCM history or realtime UI computation.
