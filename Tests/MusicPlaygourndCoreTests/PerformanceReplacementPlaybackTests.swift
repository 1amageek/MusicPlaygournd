...
struct PerformanceReplacementPlaybackTests {
    ...
    func preparedSwitchChangesOnNextCallbackAndPreservesPhase() throws {
        let transport = AudioTransport()
        let first = makeLoop(scale: 0.2)
        let second = makeLoop(scale: 0.8, beatCount: 8)
        transport.beginUpdate(revision: 42)
        try transport.submit(loop: first, revision: 42)
        try transport.installSwitchLoops([first, second], initialIndex: 0, revision: 42)
        try transport.startPlayback()
        _ = try render(transport, frames: 2_000)
        let before = transport.positionSnapshot().accumulatedBeatPosition
        let beforeSelection = transport.snapshot()
        try transport.selectSwitchLoop(index: 1, revision: 42, generation: 1)
        #expect(transport.snapshot() == beforeSelection)
        let frames = try render(transport, frames: AudioTransport.crossfadeFrames)
        let delta = first.bpm / 60 / first.sampleRate
        #expect(abs(frames[0] - sample(loop: first, beat: before)) < 0.00001)
        #expect(frames[1] > sample(loop: first, beat: before + delta))
        #expect(abs(frames.last! - sample(loop: second,
            beat: before + Double(AudioTransport.crossfadeFrames - 1) * delta)) < 0.00001)
        #expect(transport.snapshot().switchVariantIndex == 1)
        #expect(transport.positionSnapshot().accumulatedBeatPosition > before)
        _ = transport.drainRetiredAndRetainedIdentities()
        try transport.selectSwitchLoop(index: 0, revision: 42, generation: 2)
        try transport.selectSwitchLoop(index: 1, revision: 42, generation: 3)
        try transport.selectSwitchLoop(index: 0, revision: 42, generation: 4)
        _ = try render(transport, frames: AudioTransport.crossfadeFrames)
        _ = transport.drainRetiredAndRetainedIdentities()
        _ = try render(transport, frames: AudioTransport.crossfadeFrames)
        #expect(transport.snapshot().switchVariantIndex == 0)
        #expect(transport.snapshot().overrideGeneration == 4)
        let stable = transport.snapshot()
        #expect(throws: PlaybackError.staleRevision(41)) {
            try transport.selectSwitchLoop(index: 1, revision: 41, generation: 5)
        }
        #expect(transport.snapshot() == stable)
    }
