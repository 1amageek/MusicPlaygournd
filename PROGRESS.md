...
- [x] LATENCY-1 Immediate/beat/bar source adoption, app preference and phase-preserving 30ms crossfade; new native timing/PCM tests passed. 32-test run exposed two old instantaneous-adoption expectations; updated contract checks plus 9 focused regressions passed. Native two-deck/audio-clock checks passed; reviewed Mutex ownership and post-fade controls `depends:none` `parallel:none`
- [ ] LATENCY-2 Cache SwiftPM binary directory per evaluator/build root; always perform incremental compilation; real multi-file edit and failure regression evidence and reviewed commit `depends:LATENCY-1` `parallel:none`
- [ ] LATENCY-INT Verify combined source-edit/audio/control path, build app and inspect persisted timing setting; push only task commits to configured upstream `depends:LATENCY-1,LATENCY-2` `parallel:none`
