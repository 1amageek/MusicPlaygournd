...
- [x] RELEASE-040-1 Updated README/changelog and packaged version to 0.4.0 (build 4); app build, signature and exact public SwiftMusic 0.5.1 pin verified. Production code unchanged; UPDATE-051 and SYNTH behavioral evidence remains valid. Commit: Prepare MusicPlayground 0.4.0 preview release `depends:none` `parallel:none`
- [ ] RELEASE-040-INT Verify release candidate metadata, dependency pin and clean task diff; commit candidate verification before tagging/publishing the verified main commit `depends:RELEASE-040-1` `parallel:none`
